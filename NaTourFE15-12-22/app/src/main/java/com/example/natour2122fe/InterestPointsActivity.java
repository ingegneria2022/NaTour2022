    ...
    private void populateMapView(List<InterestPoints> interestPoints, GoogleMap googleMap) {
        ...
        if(interestPoints.size()!= 0) {
            ...
            for (int i = 0; i <= interestPoints.size() - 1; i++) {
                Marker startMarker = map.addMarker(new MarkerOptions().position(new LatLng(interestPoints.get(i).getLatitude(), interestPoints.get(i).getLongitude())).title("ip: " + interestPoints.get(i).getName() + "").draggable(false));
            }
